...
export class SearchService {
      ...
      if (enableAISearch && this.shouldUseAISearch(query, filters)) {
        try {
          ...
          const searchResult: SearchResult = {
            ...
            searchTime: aiResponse.processingTime,
          ...
          await this.recordSearchAnalytics({
            ...
            aiFeaturesUsed: aiResponse.featuresUsed || []
          });
  ...
  private applyTextSearch(courses: Course[], query: string): Course[] {
    return courses.filter((course) => {
      const searchableText = `
        ...
        ${course.modules.map(m => m.title + ' ' + m.description).join(' ').toLowerCase()}
      `;
  ...
  private applyFilters(courses: Course[], filters: SearchFilter): Course[] {
    return courses.filter((course) => {
      ...
      // Tenant filter for multi-tenant architecture
      if (filters.tenantId && course.tenantId !== filters.tenantId) {
        return false;
      }

      // Featured filter
      if (filters.featured !== undefined && course.featured !== filters.featured) {
        return false;
      }
  ...
  private sortResults(
    ...
    switch (sortBy) {
      ...
      case 'popular':
        ...
        break;
      case 'duration':
        sorted.sort((a, b) => a.metadata.duration - b.metadata.duration);
        break;
      case 'relevance':
      default:
   ...
   */
  async addCourseToIndex(course: Course): Promise<void> {
    try {
      this.courseDatabase[course.id] = course;
      await this.aiSearchService.indexCourse(course);
      logger.info(`Course added to search index: ${course.id}`);
    } catch (error) {
      logger.error('Error adding course to search index', error);
    }
  }
   ...
   */
  async removeCourseFromIndex(courseId: string): Promise<void> {
    try {
      delete this.courseDatabase[courseId];
      await this.aiSearchService.removeCourseFromIndex(courseId);
      logger.info(`Course removed from search index: ${courseId}`);
    } catch (error) {
      logger.error('Error removing course from search index', error);
    }
  }
  ...
  async rebuildSearchIndex(): Promise<void> {
    try {
      await this.aiSearchService.rebuildIndex();
